...
from goods.views import product_list
...
class TestProductListViews(TestCase):
    def setUp(self):
        ...
        self.url_with_category = reverse('goods:product_list_by_category', kwargs={'category_slug': self.category.slug})
    ...
    def test_product_list_view_with_invalid_category(self):
        invalid_url = reverse('goods:product_list_by_category', kwargs={'category_slug': 'invalid-slug'})
...
class SearchResultsListViewTestCase(TestCase):
    ...
    def test_valid_search_query(self):
        ...
        self.assertQuerysetEqual(response.context['goods'].values_list('name', flat=True),['Test Product'])
...
class ProductDetailViewTestCase(TestCase):
    def setUp(self):
        ...
        self.product = Product.objects.create(name='Test Product', slug='test-product', category=category, price=10, available=True)

    def test_product_detail_view(self):
        response = self.client.get(reverse('goods:product_detail', args=[self.product.id, self.product.slug]))
        self.assertEqual(response.status_code, 200)
